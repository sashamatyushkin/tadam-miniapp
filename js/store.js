...
import { CONFIG, WHEEL } from './config.js?v=2609091153';
...
async function saveCloud(json) {
  ...
  if (parts.length > 40) {                              // защита от переполнения — не пишем
    if (!warnedOverflow) {
      warnedOverflow = true;
      import('./ui.js?v=2609091153').then(m => m.toast('Данных стало много — почисти старые вишлисты, иначе новое не сохранится'));
    }
    return;
  }
